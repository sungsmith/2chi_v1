...
| **Mascot** | Cloud puff (lavender default) |
...
- **Cloud mascot** (`.mascot-cloud`) is the brand character — lavender default, with mint/pink/periwinkle tonal variants. It appears small in the sidebar (3/5 progress nudge) and large in welcome modals.
...
- **Wave hand emoji** on the dashboard greeting (`👋`) animates a small wave on a 2.8s loop — the *one* permitted celebratory motion.
...
- 220px fixed left sidebar + 64px sticky top header + 1280-max main column. Cover-letter editor extends to 1440.
- 12-column grid · 16px gutters · 48px page padding at desktop. Mobile drops to 4 columns / 12px gutter / 16px padding and the sidebar becomes a bottom tab bar (5 items).
...
- **Emoji is not part of the icon system.** A small wave 👋 appears once on the greeting and `✦` (used as a glyph) appears on the gradient AI button — that's the entire emoji budget. Don't add more.
...
| `assets/` | Logo SVGs + brand mood frames. |
| `preview/` | The small specimen cards rendered in the Design System tab. |
...
Each UI kit:
- `ui_kits/web/` — Next.js 2chi product. Dashboard, onboarding, cover-letter editor, posting capture, career editor.
